//! The RuntimeInfo encapsulates all information lksystem needs to do its job. The units, the pid and filedescriptors and the lksystem config.
//! In the lifetime of lksystem there will only ever be one RuntimeInfo which is passed wrapped inside the ArcMutRuntimeInfo.
//!
