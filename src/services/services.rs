...
impl std::fmt::Display for ServiceErrorReason {
    fn fmt(&self, fmt: &mut std::fmt::Formatter) -> std::fmt::Result {
        let msg = match self {
            ...
            ServiceErrorReason::ReloadFailed(e) => format!("Reload failed: {}", e),
...
impl Service {
    pub fn start(
        ...
        if source.is_socket_activation() || conf.sockets.is_empty() {
            ui::log(format!("Start service {}", name));
    ...
    pub fn kill_all_remaining_processes(&self, conf: &ServiceConfig, name: &str) {
        ui::log(format!("Kill all process for {}", name));
        if let Some(proc_group) = self.process_group {
            // TODO handle these errors
            match nix::sys::signal::kill(proc_group, nix::sys::signal::Signal::SIGKILL) {
                Ok(_) => ui::log(format!("Success killing process group for service {}", name,)),
                Err(e) => ui::error(format!("Error killing process group for service {}: {}", name, e,)),
            }
        } else {
            ui::log(format!("Tried to kill service that didn't have a process-group. This might have resulted in orphan processes."));
        }
        match super::kill_os_specific::kill(conf, nix::sys::signal::Signal::SIGKILL) {
            Ok(_) => ui::log(format!("Success killing process os specificly for service {}", name,)),
    ...
    pub fn reload(
        ...
        let Some(reload_cmd) = conf.reload.as_ref() else {
            return Ok(());
        };

        let timeout = self.get_start_timeout(conf);
        self.run_cmd(reload_cmd, id, name, timeout, run_info)
            .map_err(ServiceErrorReason::ReloadFailed)
    }
    pub fn kill(
        &mut self,
        conf: &ServiceConfig,
        ...
        self.stop(conf, id.clone(), name, run_info)
            ...
            .and_then(|r| {
                // After successful stop and poststop, try to remove the service cgroup (best-effort)
                #[cfg(target_os = "linux")]
                {
                    let _ = crate::platform::cgroups::remove_cgroup(&conf.platform_specific.cgroup_path);
                }
                Ok(r)
            })
    }
    ...
    fn run_cmd(
        ...
        let stdout = if let Some(stdio) = &self.stdout {
            unsafe {
                ...
                Stdio::from(std::fs::File::from(duped))
            }
        } else {
            Stdio::piped()
        };
        ...
        ui::log(format!("Run {:?} for service: {}", cmdline, name));
        let spawn_result = {
            let mut pid_table_locked = run_info.pid_table.lock().unwrap();
            let res = cmd.spawn();
            if let Ok(child) = &res {
        ...
        match spawn_result {
            Ok(mut child) => {
                ui::log(format!("Wait for {:?} for service: {}", cmdline, name));
                let wait_result: Result<(), RunCmdError> = match wait_for_helper_child(
                    ...
                    WaitResult::InTime(Ok(exitstatus)) => {
                        if exitstatus.success() {
                            ui::log(format!("success running {:?} for service: {}", cmdline, name));
                            Ok(())
                        } else {
                    ...
                    WaitResult::TimedOut => {
                        ui::log(format!("Timeout running {:?} for service: {}", cmdline, name));
                        let _ = child.kill();
    ...
    fn run_poststop(
        ...
        ui::log(format!("Run poststop for {}", name));
        let timeout = self.get_stop_timeout(conf);
    ...
    pub fn log_stdout_lines(&mut self, name: &str, status: &UnitStatus) -> std::io::Result<()> {
        while self.stdout_buffer.contains(&b'\n') {
            ...
            let message = format!("[{}][{:?}] {}", name, *status, String::from_utf8_lossy(line));
    ...
    pub fn log_stderr_lines(&mut self, name: &str, status: &UnitStatus) -> std::io::Result<()> {
        while self.stderr_buffer.contains(&b'\n') {
            ...
            let message = format!("[{}][{:?}][STDERR] {}", name, *status, String::from_utf8_lossy(line));
...
fn wait_for_helper_child(
    ...
    loop {
        ...
        {
            let mut pid_table_locked = run_info.pid_table.lock().unwrap();
            let maybe_entry = pid_table_locked.get(&pid).cloned();
            if let Some(entry) = maybe_entry {
                match entry {
                    PidEntry::HelperExited(termination) => {
                        let _ = pid_table_locked.remove(&pid);
                        return WaitResult::InTime(Ok(termination));
                    }
                    PidEntry::Helper(_, _) => {}
                    PidEntry::Service(_, _) | PidEntry::ServiceExited(_) => {
                        unreachable!("Was waiting on helper process but pid got saved as a service entry")
                    }
                }
            }
        }
        match child.try_wait() {
            Ok(Some(status)) => {
                let termination = match status.code() {
                    Some(code) => crate::signal_handler::ChildTermination::Exit(code),
                    None => crate::signal_handler::ChildTermination::Signal(
            ...
            Err(err) => {
                if let Some(errno) = err.raw_os_error() {
                    if errno == nix::errno::Errno::ECHILD as i32 {
                        // The child may already have been reaped by the SIGCHLD handler.
                        // Treat this as a completed helper and return success.
                        return WaitResult::InTime(Ok(crate::signal_handler::ChildTermination::Exit(0)));
                    } else {
                        return WaitResult::InTime(Err(std::io::Error::new(
                            std::io::ErrorKind::Other,
                            format!("failed to wait for helper child: {err}"),
                        )));
                    }
                }
            }
