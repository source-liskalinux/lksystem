...
fn usage() -> ! {
    ...
    println!("--------------------------------");
    println!("");
    println!("Usage: lksysctl <options> [command] <service....>");
    println!("> reboot      | restart             restart the whole system");
    println!("> shutdown    | poweroff            power off the whole system");
    println!("> halt                              halt the whole system");
    println!("> up <serv>   | start <serv>        bring the service up, wait until running");
    println!("> down <serv> | stop <serv>         bring the service down, wait until stopped");
    println!("> exit <serv> | shutdown <serv>     send exit, wait until stopped");
    println!("> once <serv>                       run once, don't restart on exit");
    println!("> pause <serv>                      pause the service (SIGSTOP)");
    println!("> cont <serv> | continue <serv>     resume a paused service (SIGCONT)");
    println!("> term <serv>                       send SIGTERM");
    println!("> kill <serv>                       send SIGKILL");
    println!("> hup <serv>                        send SIGHUP");
    println!("> alarm <serv>                      send SIGALRM");
    println!("> int <serv>                        send SIGINT");
    println!("> quit <serv>                       send SIGQUIT");
    println!("> usr1 <serv>                       send SIGUSR1");
    println!("> usr2 <serv>                       send SIGUSR2");
    println!("> restart <serv>                    term, then bring back up, wait until running");
    println!("> force-stop <serv>                 down, then kill if it doesn't stop in time");
    println!("> force-restart <serv>              term + kill, then bring back up");
    println!("> force-shutdown <serv>             exit, then kill if it doesn't stop in time");
    println!("> status <serv>                     show current state of the service");
    println!("> -v                                verbose: print the resulting status after each action");
    println!("> -w seconds                        how long to wait for the desired state (default 7 or $LKSYSCTL_WAIT)");
    println!("");
