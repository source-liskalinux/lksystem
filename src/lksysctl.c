...
int svstatus_get() {
  if ((fd =open_write("supervise/ok")) == -1) {
    if (errno == error_nodevice) {
      *acts == 'x' ? ok("lksys not running") : failx("lksys not running");
      return(0);
    }
    if (islog && (errno == error_noent))
      warnx("supervise/ok not found, exit and restart lksys to enable log");
    else
...
int control(char *a) {
  if ((fd =open_write("supervise/control")) == -1) {
    if (errno != error_nodevice)
      ...
      *a == 'x' ? ok("lksys not running") : failx("lksys not running");
    return(-1);
  }
...
int main(int argc, char **argv) {
  ...
  *argv +=i;
  ...
  service =argv;
  services =1;
  lsb =str_diff(progname, "lksysctl");
  if ((x =env_get("LKSYSTEM_SERVICE_DIR"))) varservice =x;
  if ((x =env_get("LKSYSCTL_WAIT"))) scan_ulong(x, &wait);
