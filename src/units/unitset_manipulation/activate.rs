...
pub enum UnitOperationErrorReason {
    ...
    ServiceReloadError(ServiceErrorReason),
...
impl std::fmt::Display for UnitOperationError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match &self.reason {
            ...
            UnitOperationErrorReason::ServiceReloadError(msg) => {
                write!(
                    f,
                    "Service {} (ID {}) failed to reload because: {}",
                    self.unit_name, self.unit_id, msg
                )?;
            }
...
pub fn activate_unit(
    ...
    ui::log(format!("Activate id: {:?}", id_to_start));
...
pub fn activate_needed_units(
    ...
    ui::log(format!("Needed units to start {:?}: {:?}", target_id, needed_ids));
    ...
    ui::log(format!("Root units found: {:?}", root_units));
    ...
    for err in &errs {
        ui::error(format!("Error while activating unit graph: {}", err));
    }
    errs
}
            ...
            match activate_unit(
                ...
                Err(e) => {
                    if let UnitOperationErrorReason::DependencyError(_) = e.reason {
                        ...
                        ui::error(format!("Error while activating unit {}", e));
