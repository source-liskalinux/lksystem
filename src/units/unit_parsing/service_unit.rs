...
pub fn parse_service(
    ...
    for (name, section) in parsed_file {
        match name.as_str() {
            ...
            "[Condition]" => {
                conditions = Some(parse_conditions(&mut section.clone())?);
            }
    ...
    Ok(ParsedServiceConfig {
        common: ParsedCommonConfig {
            ...
            conditions: conditions.unwrap_or_else(Default::default),
        },
        srvc: service_config,
    })
}
...
impl Timeout {
    fn into_duration(self) -> std::time::Duration {
        match self {
            ...
            Timeout::Infinity => std::time::Duration::MAX,
        }
    }
}
...
fn parse_service_section(
    ...
    let starttimeout = match starttimeout {
        Some(vec) => {
            if vec.len() == 1 {
                Some(parse_timeout(&vec.last().unwrap().1))
            } else {
    ...
    let srcv_type = match srcv_type {
        Some(vec) => {
            if vec.len() == 1 {
                match vec.last().unwrap().1.as_str() {
    ...
    let cpu_weight = match cpu_weight {
        Some(mut vec) => {
            if vec.len() == 1 {
                let value = vec.remove(0).1;
                Some(value.parse::<u64>().map_err(|_| {
                    ParsingErrorReason::Generic(format!(
                        "Could not parse CPUWeight value: {}",
                        value
                    ))
                })?)
            } else {
    ...
    let io_weight = match io_weight {
        Some(mut vec) => {
            if vec.len() == 1 {
                Some(vec.remove(0).1.parse::<u64>().map_err(|_| {
                    ParsingErrorReason::Generic(format!(
                        "Could not parse IOWeight value: {}",
                        vec[0].1
                    ))
                })?)
            } else {
    ...
    let remain_after_exit = match remain_after_exit {
        Some(vec) => {
            if vec.len() == 1 {
                string_to_bool(&vec[0].1)
            } else {
                return Err(ParsingErrorReason::SettingTooManyValues(
                    "RemainAfterExit".to_owned(),
                    super::map_tupels_to_second(vec),
                ));
            }
        }
    ...
    let restart_sec = match restart_sec {
        Some(vec) => {
            if vec.len() == 1 {
                Some(parse_timeout(&vec[0].1).into_duration())
            } else {
                return Err(ParsingErrorReason::SettingTooManyValues(
                    "RestartSec".to_owned(),
                    super::map_tupels_to_second(vec),
