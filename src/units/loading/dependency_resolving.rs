...
pub fn prune_units(
    ...
    for id in &ids_to_remove {
        ...
        ui::log(format!("Pruning unit: {}", unit.id.name));
    }
    ...
    for unit in unit_table.values_mut() {
        match &mut unit.specific {
            ...
            Specific::Device(_) => { /**/ }
            Specific::Timer(_) => {
                // Note: unlike sockets/services above, a timer's `Unit=`
                // reference isn't filtered against `ids_to_keep` here -- if
                // its target got pruned, the timer will just fail to
                // activate it at runtime (traced, not fatal). Good enough
                // for now; revisit if that turns out to be surprising.
            }
            Specific::Mount(_) => { /**/ }
            Specific::Path(path) => {
                if !ids_to_keep.contains(&path.conf.unit) {
                    // The referenced target unit was pruned, so this path unit
                    // cannot activate. Keep it around if the only reason it was
                    // included was to satisfy dependency traversal, but do not
                    // alter its config.
                }
            }
        }
...
pub fn fill_dependencies(units: &mut HashMap<UnitId, Unit>) -> Result<(), String> {
    ...
    for unit in (*units).values_mut() {
        ui::log(format!("Fill deps for unit: {:?}", unit.id));
    ...
    for (wanted, wanting) in wanted_by {
        ui::log(format!("{:?} wants {:?}", wanting, wanted));
        let unit = units.get_mut(&wanting).ok_or_else(|| {
            format!(
                "Unit {:?} has WantedBy/RequiredBy reference to unknown unit {:?}",
                wanting, wanted
            )
        })?;
        unit.common.dependencies.wants.push(wanted.clone());
        let unit = units.get_mut(&wanted).ok_or_else(|| {
            format!(
                "Unit {:?} has WantedBy/RequiredBy reference to unknown unit {:?}",
                wanting, wanted
            )
    ...
    for (required, requiring) in required_by {
        let unit = units.get_mut(&requiring).ok_or_else(|| {
            format!(
                "Unit {:?} has Requires/RequiredBy reference to unknown unit {:?}",
                requiring, required
            )
        })?;
        unit.common.dependencies.requires.push(required.clone());
        let unit = units.get_mut(&required).ok_or_else(|| {
            format!(
                "Unit {:?} has Requires/RequiredBy reference to unknown unit {:?}",
                requiring, required
            )
    ...
    for (before, after) in before {
        let unit = units.get_mut(&after).ok_or_else(|| {
            format!(
                "Unit {:?} has Before/After reference to unknown unit {:?}",
                before, after
            )
        })?;
    ...
    for (after, before) in after {
        let unit = units.get_mut(&before).ok_or_else(|| {
            format!(
                "Unit {:?} has Before/After reference to unknown unit {:?}",
                after, before
            )
...
fn apply_sockets_to_services(unit_table: &mut UnitTable) -> Result<(), String> {
    ...
    for sock_unit in &socket_ids {
        ...
        if let Specific::Socket(sock) = &mut sock_unit.specific {
            ui::log(format!("Searching services for socket: {}", sock_unit.id.name));
        ...
        if counter == 0 {
            ui::warning(format!("Added socket: {} to no service", sock_name));
        } else if counter > 1 {
            ui::warning(format!(
                "Added socket: {} to multiple services: {}",
                sock_name, counter
            ));
        }
    }
