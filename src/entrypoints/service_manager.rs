...
use crate::config;
use crate::control;
use crate::device_events;
use crate::logging;
use crate::notification_handler;
use crate::platform;
use crate::path_events;
...
pub fn run_service_manager() {
    ...
    let conf = match conf {
        Ok(conf) => conf,
        Err(e) => {
            ui::error(format!("Error while loading the conf: {}", e));
            unrecoverable_error(format!(
                "Reading conf did not work. See stdout or log at: {:?}",
                log_conf.log_dir
            ));
            // unrecoverable_error always shuts down lksystem
            unreachable!("");
        }
    };
    #[cfg(target_os = "linux")]
    ...
    // start listening for kernel device events before coldplug, so the coldplug
    // pass's re-triggered uevents actually get caught instead of racing ahead of
    // the listener existing.
    device_events::start_device_events_thread(run_info.clone());
    device_events::coldplug();
    // also start the timer scheduler before any unit gets activated, so
    // `timer_events::boot_instant()` (which OnBootSec= is measured from)
    // reflects "lksystem is ready to run timers", not some later point.
    timer_events::start_timer_thread(run_info.clone());
    path_events::start_path_events_thread(run_info.clone());
    ui::log(format!("Started all helper threads. Start activating units"));
...
fn unrecoverable_error(error: String) {
    if nix::unistd::getpid().as_raw() == 1 {
        crate::ui::error(format!("Unrecoverable error: {}", error));
                ...
                Ok(mut child) => match child.wait() {
                    Ok(_) => {
                        ...
                        crate::ui::warning(format!(
                            "Returned from shell. Will exit after sleeping: {:?}",
                            dur
                        ));
...
fn move_to_new_session() -> bool {
    match unsafe { nix::unistd::fork() } {
        ...
        Err(e) => {
            ui::error(format!("Fork before setsid failed: {}", e));
            false
        }
    }
}
...
fn remount_root_rw() {
    ...
    if let Err(e) = nix::mount::mount(source, "/", fs_type, flags, data) {
        ui::error(format!("Failed to remount root as rw: {}", e));
    }
}

// mount basic virtual filesystem that must available before other services running
// normally. kernel not mount this automatically as PID1, so we must mount it manually 
// in the init process like systemd when booting
#[cfg(target_os = "linux")]
fn mount_early_virtual_filesystems() {
    ...
    let targets: &[(&str, &str, &str, MsFlags)] = &[
        ...
        ("tmpfs", "/dev", "tmpfs", MsFlags::MS_NOSUID),
        ("devpts", "/dev/pts", "devpts", MsFlags::MS_NOSUID | MsFlags::MS_NOEXEC),
        ("tmpfs", "/run", "tmpfs", MsFlags::MS_NOSUID | MsFlags::MS_NODEV),
    ];
    for (src, target, fstype, flags) in targets {
        let _ = std::fs::create_dir_all(target);
        if let Err(e) = mount(Some(*src), *target, Some(*fstype), *flags, None::<&str>) {
            if e != nix::errno::Errno::EBUSY {
                ui::error(format!("Failed to mount {} ({}): {}", target, fstype, e));
            }
        } else {
            ui::log(format!("Successfully mounted {} ({})", target, fstype));
        }
    }
}

// RUSTYSD OLD BUG: `getpid() == 0`, though PID1 PID value is 1 (PID 0 is not available in 
// Linux userspace). Then this made PID1 setup (first mount, remount
// root) NEVER RUNNING even if exec as init=.
#[cfg(target_os = "linux")]
fn pid1_specific_setup() {
    if nix::unistd::getpid().as_raw() == 1 {
        mount_early_virtual_filesystems();
        remount_root_rw();
    }
}
...
fn prepare_runtimeinfo(conf: &config::Config, dry_run: bool) -> runtime_info::ArcMutRuntimeInfo {
    ...
    ui::log(format!("Finished loading units"));
    if let Err(e) = units::sanity_check_dependencies(&unit_table) {
        match e {
            units::SanityCheckError::CirclesFound(circles) => {
                ui::error(format!("Found {} cycle(s) in the dependencies", circles.len()));
                for circle in &circles {
                    ui::error(format!("-- Next circle --"));
                    for id in circle {
                        ui::error(format!("{}", id));
                    }
                    ui::error(format!("-- End circle --"));
                }
            }
            units::SanityCheckError::Generic(msg) => {
                ui::error(format!("Unit dependencies did not pass sanity checks: {}", msg));
            }
        }
        unrecoverable_error("Unit dependencies did not pass sanity check".into());
    }
    ui::log(format!("Unit dependencies passed sanity checks"));
    let unit_table = unit_table;
    if dry_run {
        ui::warning(format!("Exit after loading because --dry-run was passed"));
