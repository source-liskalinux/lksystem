...
#define VERSION "$Id: bcc4ba63124854a6c268d994a6895a01e26cc62d $"

char *progname;
char *lksysdir ="/etc/lksystem/lksysdir";
char *new;
...
int main (int argc, char **argv) {
  ...
  new =*argv;
  if ((x =env_get("lksysDIR"))) lksysdir =x;
  if (new[0] == '.') fatalx(new, ": must not start with a dot.");
  if (chdir(lksysdir) == -1) fatal("unable to chdir: ", lksysdir);
  ...
  if ((s.st_ino == ino) && (s.st_dev == dev)) {
    buffer_puts(buffer_1, "lksyschdir: ");
