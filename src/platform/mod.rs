...
// netlink(7)/NETLINK_KOBJECT_UEVENT is a Linux-only kernel interface (there is
// no equivalent uevent multicast group on the BSDs), so this module is gated
// the same way unix_common's re-export already is below -- it simply won't be
// part of the build on non-Linux targets, and .device units fall back to
// "unsupported on this platform" at the unit-loading layer (see the
// architecture doc for where that check lives).
#[cfg(target_os = "linux")]
mod netlink_uevent;
#[cfg(target_os = "linux")]
pub use netlink_uevent::*;
...
mod tests {
    #[test]
    fn cgroups_are_enabled_by_default() {
        assert!(cfg!(feature = "cgroups"), "cgroups support should be enabled by default");
    }
}
