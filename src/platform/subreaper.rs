...
pub fn become_subreaper(set: bool) {
    unsafe {
        ...
        if res < 0 {
            crate::ui::error("Couldnt set subreaper for lksystem");
            return;
        }
    }
}
...
pub fn become_subreaper(set: bool) {
    unsafe {
        // set subreaper to collect all zombies left behind by the services
        // make pull request to libc to include this
        let res = if set {
            procctl(
                libc::P_PID as i32,
                libc::getpid(),
        ...
        if res < 0 {
            ...
            crate::ui::success("Acquire/Release subreaper privilege successfully");
        }
    }
}
